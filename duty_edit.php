...
<body>
	...
	<main id="main">
		...
		<?php
		include('link.php');
		$result = mysqli_query($con,"SELECT * FROM duty WHERE id='" . $_GET['id'] . "'");
		$row= mysqli_fetch_array($result);
		if(count($_POST)>0) {

			mysqli_query($con,"UPDATE duty set date_at='" . $_POST['date'] . "', remark='" . $_POST['remarks'] . "', hours='" . $_POST['hour'] . "' WHERE id='" . $_POST['userId'] . "'");
					...
					<form action="" name="frmUser" method="POST" enctype="multipart/form-data">
						...
						<div class="form-group">
							<div class="input-group">
								...
								<input id="datepicker" class="keyboard form-control" id="validationDefaultUsername" name="date" placeholder="date....." aria-describedby="inputGroupPrepend2"  value="<?php echo $row['date_at']?>" autocomplete="off" required >
							</div>
						</div>
						<!-- {{-- hours --}} -->
						<div class="form-group">
							<div class="input-group">
								...
								<input class="form-control" id="validationDefaultUsername" name="hour" placeholder="hours ... " aria-describedby="inputGroupPrepend2" value="<?php echo $row['hours']?>"required>
							</div>
