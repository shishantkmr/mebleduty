...
$year_month = mysqli_query($con,"SELECT *  FROM duty where user_id = $user_id  GROUP BY YEAR(date_at), MONTH(date_at) order by date_at desc"); // just for month

$year_months = mysqli_query($con,"SELECT *  FROM duty where user_id = $user_id  GROUP BY YEAR(date_at) order by date_at desc"); // Year for selection box
...
function showUser(str) {
  ...
  xmlhttp.open("GET","getamount_onmonth.php?q="+str,true);
  xmlhttp.send();
}
</script>
					...
					<div class="form-group col-md-4">
						...
						<select id="inputState" name ="month" class="form-control" size="1"  name="users" onchange="showUser(this.value)">
							...
							while($row = mysqli_fetch_array($year_month)) {
								...
								?>
								<option value="<?php echo $row['date_at'];?>"><?php  echo date("F, Y ",strtotime($row['date_at']));?> </option>
							<?php } ?>
							<?php
							while($row = mysqli_fetch_array($year_months)) {
								...
								?>
								<option value="<?php  echo date("Y ",strtotime($row['date_at']));?>"><?php  echo date("Y ",strtotime($row['date_at']));?> Total Hours </option>
							<?php } ?>
